...
import at.technikum.frontend.controllers.BaseLogController;
...
public class LogValidator extends Validator {
  ...
  private boolean validateStartDate(final LogViewModel logViewModel) {
    if (logViewModel.getStartDate() == null) {
      ...
      return false;
    } else if (!validateDateRange(logViewModel)) {
  ...
  private boolean validateDateRange(LogViewModel logViewModel) {
    if (logViewModel.getStartDate() == null || logViewModel.getEndDate() == null) {
      return false;
    }
    if (logViewModel.getEndDate().isAfter(logViewModel.getStartDate())) {
      return true;
    } else {
      return false;
    }
