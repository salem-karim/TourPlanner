...
public class RequestHandler {
   ...
   * @return an Optional Object of an 2 sized Double Array
   *         Gets Coordinates from a Location String using ORS' geocode endpoint
   */
   ...
   * @return A {@link JsonNode} representing the rout response of the API
   *         or null if routing failed or no route was found.
   */
  public JsonNode RouteBetween(final Optional<double[]> fromCoordsOpt, final Optional<double[]> toCoordsOpt,
    ...
    try {
      final String body = String.format("""
          ...
          """, fromCoords[0], fromCoords[1], toCoords[0], toCoords[0]);
  ...
  public Optional<RouteInfo> parseRouteInfo(JsonNode json) {
    if (json != null && json.has("routes") && json.get("routes").size() > 0) {
